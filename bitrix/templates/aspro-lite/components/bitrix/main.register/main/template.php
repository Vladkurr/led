...
<label style="text-align: center; font-size: 20px; padding-bottom: 20px" class="error"><?= $_GET["create"] == "Y" ? "Номер не найден, создайте аккаунт, пожалуйста" : ""?></label>
<?php if($_REQUEST["SMS"] == "Y"): ?>
<div class="registraion-page pk-page">
    <div class="form form--send-sms" style="max-width: 484px;padding: 0; margin: auto;">
        <div class="form-header">
            <div class="text">
                ...
                <div class="form_desc font_16">Мы отправили код подтверждения на
                    номер <span>+<?= $_REQUEST['PHONE'] ?></span></div>
            </div>
        </div>
        <form id="registraion-page-form" method="post" action="" name="regform">
        ...
        <div class="form-footer">
                <button class="btn btn-default btn-wide btn-sms" name="code_submit_button">Отправить код</button>
                <div id="bx_auth_resend_89popup_text">Вы сможете отправить код повторно через <span id="bx_auth_resend_89popup_counter">60</span> сек.</div>
            </div>
        <div id="bx_register_error" style="display:none"><? ShowError('error') ?></div>
        <div id="bx_register_resend"></div>
        <script>
        document.addEventListener("DOMContentLoaded", async () => {
            //url = "/ajax/send_sms_ajax.php?PHONE=<?php //= $_REQUEST['PHONE'] ?>//"
       ...
       document.querySelector("#registraion-page-form").addEventListener("submit", (e)=>{
            e.preventDefault()
        })

        document.querySelector("#input_SMS_CODE").addEventListener("keyup", async (e) => {
            if(e.target.value == window.code?.value) {
                url = "/ajax/create_user.php?PHONE=<?= $_REQUEST['PHONE'] ?>"
                let result = await fetch(url, {
                    method: 'POST',
                    body: {},
                })
                    window.location.replace(`https://ledspro.ru/personal/`)
            }
        })

        document.querySelector(".btn-sms").addEventListener("click", async (e)=>{
            e.target.style = "display:none;";
            url = "/ajax/send_sms_ajax.php?PHONE=<?= $_REQUEST['PHONE'] ?>"
            let result = await fetch(url, {
                method: 'POST',
                body: {},
            }).then(response => {
            ...
            if(result.status == 200){
                count = 60;
                if(document.querySelector("#bx_auth_resend_89popup_counter")){
                        ...
                        function myLoop () {           //  create a loop function
                           setTimeout(function () {    //  call a 3s setTimeout when the loop is called
                              ...
                              if(count == 0){
                                   ...
                                   document.querySelector("#bx_auth_resend_89popup_counter").innerHTML = 60;
                                   return;
                               }
                              document.querySelector("#bx_auth_resend_89popup_counter").innerHTML = count;
                              i++;                     //  increment the counter
                              if (i < 61) {            //  if the counter < 10, call the loop function
    ...
    <div class="form">
        ...
        <? else: ?>
            <form id="registraion-page-form" method="post" action="<?= POST_FORM_ACTION_URI ?>" name="regform"
                ...
                <div class="form-body">
                    <? foreach ($arFields as $FIELD): ?>
                        ...
                        <? if (
                            ...
                            <?php if ($FIELD != "PASSWORD" && $FIELD != "CONFIRM_PASSWORD"): ?>
                                <label class="font_14" for="input_<?= $FIELD ?>">
                                    <?= (($arTheme['CABINET']['DEPENDENT_PARAMS']["PERSONAL_ONEFIO"]["VALUE"] != "N" && $FIELD == "NAME") ? GetMessage("REGISTER_FIELD_ONENAME") : GetMessage("REGISTER_FIELD_" . $FIELD)); ?> <? if ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y"): ?>
                                        <span class="required-star">*</span><? endif; ?>
                                </label>
                            <?php endif; ?>
                        ...
                        <? switch ($FIELD) {
                            case "PASSWORD":
                                ?>
                                <input size="30" type="hidden" id="input_<?= $FIELD; ?>"
                                       name="REGISTER[<?= $FIELD ?>]" required
                                       value="<?= $arResult["VALUES"][$FIELD] ?>" autocomplete="off"
                                       class="form-control password <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>"/>

                                <? break;
                            case "CONFIRM_PASSWORD":
                                ?>
                                <input size="30" id="input_<?= $FIELD; ?>" name="REGISTER[<?= $FIELD ?>]" required
                                       value="<?= $arResult["VALUES"][$FIELD] ?>" autocomplete="off"
                                       class="form-control confirm_password <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>"
                                       type="hidden"/>
                            ...
                            case "PERSONAL_PHOTO":
                            case "WORK_LOGO": ?>
                                <input size="30" type="file" class="form-control" id="input_<?= $FIELD; ?>"
                                       name="REGISTER_FILES_<?= $FIELD ?>"/>
                                <? break;
                            case "PERSONAL_NOTES":
                            case "WORK_NOTES": ?>
                                <textarea cols="30" rows="5" class="form-control" id="input_<?= $FIELD; ?>"
                            ...
                            <? case "EMAIL": ?>
                                <input size="30" type="email" id="input_<?= $FIELD; ?>"
                                       name="REGISTER[<?= $FIELD ?>]" <?= ($arResult["EMAIL_REQUIRED"] || in_array($FIELD, $arResult["REQUIRED_FIELDS"]) ? "required" : ""); ?>
                                       value="<?= $arResult["VALUES"][$FIELD] ?>" class="<?= $class ?>"
                                       id="emails"/>
                                <? break; ?>
                            <? case "NAME": ?>
                            ...
                            <? case "PERSONAL_PHONE": ?>
                                <input size="30" type="text" id="input_<?= $FIELD; ?>"
                                       name="REGISTER[<?= $FIELD ?>]"
                                       class="form-control phone_input <?= (array_key_exists($FIELD, $arResult["ERRORS"])) ? 'error' : '' ?>" <?= ($arResult["REQUIRED_FIELDS_FLAGS"][$FIELD] == "Y" ? "required" : ""); ?>
                            ...
                            default:
                                ...
                                <? // hide login
                                ?>
                                <input size="30" id="input_<?= $FIELD; ?>"
                                       class="form-control" <?= (($FIELD == "LOGIN" && $arTheme['CABINET']['DEPENDENT_PARAMS']["LOGIN_EQUAL_EMAIL"]["VALUE"] == "Y") ? 'type="hidden" value="1"' : 'type="text"'); ?>
                        ...
                        <? if (
                            ...
                            <? endif; ?>
                            <?php if($FIELD != "CONFIRM_PASSWORD" && $FIELD != "PASSWORD"): ?>
                    ...
                    <? if ($arUFields): ?>
                        <? foreach ($arUFields as $arUField): ?>
                            <div class="r">
                                <label><span><?= $arUField["EDIT_FORM_LABEL"]; ?>&nbsp;<? if ($arUField["MANDATORY"] == "Y"): ?>
                                            <span class="required-star">*</span><? endif; ?></span></label>
                                <? $APPLICATION->IncludeComponent(
                    ...
                    <? if ($arResult["USE_CAPTCHA"] == "Y"): ?>
                        ...
                        <div class="form-control captcha-row clearfix">
                            <label for="captcha_word"
                                   class="font_14"><span><?= (TSolution\ReCaptcha::checkRecaptchaActive() ? GetMessage("FORM_GENERAL_RECAPTCHA") : GetMessage("REGISTER_CAPTCHA_PROMT")) ?>&nbsp;<span
                                            class="required-star">*</span></span></label>
                            <div class="captcha_image">
                                <img src="/bitrix/tools/captcha.php?captcha_sid=<?= htmlspecialcharsbx($arResult["CAPTCHA_CODE"]) ?>"
                                ...
                                <span class="refresh"><a href="javascript:;"
                                                         rel="nofollow"><?= GetMessage("REFRESH") ?></a></span>
                            </div>
                            <div class="captcha_input">
                                <input type="text" class="inputtext form-control captcha" name="captcha_word"
                ...
                <div class="form-footer">
                    <button class="btn btn-default btn-lg btn-wide" type="submit" name="register_submit_button1"
                    ...
                    <? if (COption::GetOptionString("aspro.lite", "SHOW_LICENCE", "N") == "Y"): ?>
                        <div class="licence_block">
                            ...
                            <input type="checkbox" id="licenses_reg" name="licenses_popup" checked required
                                   value="Y" class="form-checkbox__input">
                        </div>
                    <? endif; ?>
                </div>
            </form>
            ...
            <script>
                document.querySelector("[name='register_submit_button1']").addEventListener("click", () => {
                    ...
                    document.querySelector("#input_LOGIN").value =  document.querySelector("#input_PERSONAL_PHONE").value
                    document.querySelector("#input_PASSWORD").value = password
                    document.querySelector("#input_CONFIRM_PASSWORD").value = password
                    console.log(document.querySelector("#input_CONFIRM_PASSWORD").value)
                })
                $(document).ready(function () {
                    ...
                    if (arAsproOptions['THEME']['PHONE_MASK'].length) {
                        ...
                        $('form#registraion-page-form input.phone_input').inputmask('mask', {
